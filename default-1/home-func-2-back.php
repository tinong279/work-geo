...
function home_get_inclinometer_all()
{
	...
	// home_get_inclinometer_with_offset("6.2K 傾斜儀(左)", 11, $id_11_ad1_offset, $id_11_ad2_offset);
	// home_get_inclinometer_with_offset("6.2K 傾斜儀(右)", 12, $id_12_ad1_offset, $id_12_ad2_offset);
}
...
function home_get_inclinometer_with_offset_11($loc_name)
{
	...
	if ($db_link == TRUE) {
		...
		$sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=1102 ORDER BY `sn` DESC LIMIT 1;";
		$stmt = $db_link->prepare($sql_query);
		if ($stmt == TRUE) {
			$stmt->execute();
			$stmt->store_result();
		...
		$sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=1105 ORDER BY `sn` DESC LIMIT 1;";
		$stmt = $db_link->prepare($sql_query);
		if ($stmt == TRUE) {
			$stmt->execute();
			$stmt->store_result();
...
function home_get_inclinometer_with_offset_12($loc_name)
{
	...
	if ($db_link == TRUE) {
		...
		$sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=1202 ORDER BY `sn` DESC LIMIT 1;";
		$stmt = $db_link->prepare($sql_query);
		if ($stmt == TRUE) {
			$stmt->execute();
			$stmt->store_result();
		...
		$sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=1205 ORDER BY `sn` DESC LIMIT 1;";
		$stmt = $db_link->prepare($sql_query);
		if ($stmt == TRUE) {
			$stmt->execute();
			$stmt->store_result();
	...
	if ($db_link == TRUE) {
		...
		if ($stmt == TRUE) {
			...
			if ($data_count == 1) {
				...
				$output .= '<td style="text-align:center;vertical-align:middle;"><span>' . get_status_circle_raingauge2(1903, strtotime($data_buf["time"])) . '</span></td>' . "\n";
