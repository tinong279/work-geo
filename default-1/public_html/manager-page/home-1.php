...
<head>
	<title>苗62鄉道 2.1K邊坡 即時監測數據 - <?php echo $system_name; ?></title>
	<meta charset="utf-8">
	...
	<script language="javascript">
		...
		function cms_on() {
			...
			$.get("api/cms-on1.php?token1=<?php echo $_SESSION['token1']; ?>", function(data, status) {
		...
		function cms_off() {
			...
			$.get("api/cms-off1.php?token1=<?php echo $_SESSION['token1']; ?>", function(data, status) {
				document.getElementById('cms_connecting').style.display = 'none';
				document.getElementById('cms_response').style.display = 'block';
...
<body>
	...
	<div class="main-content w3-main w3-container w3-padding-large">
		<!-- style="margin-left:240px;margin-top:59px;" -->

		<div class="main-card">
			<h1 style="color: #ffffff;">苗62鄉道 2.1K邊坡 即時監測數據</h1>
		</div>
		...
		<div class="main-card w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="picture-container">
				<div class="w3-half">
					<a href="/manager-page/api/live.php?id=11" target="_blank">
						<img id="img_cam1" loading="lazy" style="width:100%;object-fit:fill;margin-top:14px;" data-src="api/snapshot.php?id=11" src="api/snapshot.php?id=11" onerror="this.src='live/1-1.jpg';" />
					</a>
				</div>
				<div class="w3-half">
					<a href="/manager-page/api/live.php?id=12" target="_blank">
						<img id="img_cam2" loading="lazy" style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;" data-src="api/snapshot.php?id=12" src="api/snapshot.php?id=12" onerror="this.src='live/1-2.jpg';" />
					</a>
				</div>
			</div>
			...
			<div class="picture-container">
				<div class="w3-half">
					<img id="img_alert1" style="width:100%;object-fit:fill;margin-top:14px;"
						src="alarm/1-1.jpg<?php echo '?t=' . rand() ?>"
						onerror="this.onerror=null; this.src='alarm/1-1.jpg?t=' + new Date().getTime();"
						loading="lazy">
				</div>
				<div class="w3-half">
					<img id="img_alert2" style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;"
						src="alarm/1-2.jpg<?php echo '?t=' . rand() ?>"
						onerror="this.onerror=null; this.src='alarm/1-2.jpg?t=' + new Date().getTime();"
						loading="lazy">
				</div>
			</div>
		</div>
	...
	<div id="locpic_01" class="w3-modal">
		<div class="w3-modal-content" style="max-width:480px;">
			<div class="w3-bar w3-blue">
				<a class="w3-bar-item w3-padding-8" style="font-size:20px; vertical-align:middle;">苗62鄉道 2.1K邊坡</a>
		...
		<script>
			...
			function refreshImagesLoop() {
				fetch('fetch_images.php?nocache=' + new Date().getTime())
					...
					.catch(err => {
						...
						document.getElementById('img_alert1').src = 'http://192.168.60.79/manager-page/alarm/1-1.jpg?t=' + fallbackTime;
						document.getElementById('img_alert2').src = 'http://192.168.60.79/manager-page/alarm/1-2.jpg?t=' + fallbackTime;
					})
					.finally(() => {
			...
			async function updateStatusLightsLoop() {
				try {
					// 第一步：先問「牌面狀態」
					try {
						let cms_res = await $.get("api/cms-status1.php");
					...
					try {
						let lp_res = await $.get("api/lp-status1.php");
