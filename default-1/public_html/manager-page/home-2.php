...
<head>
	<title>苗62鄉道 6.2K邊坡 即時監測數據 - <?php echo $system_name; ?></title>
	<meta charset="utf-8">
	...
	<script language="javascript">
		...
		function cms_on() {
			...
			$.get("api/cms-on2.php?token1=<?php echo $_SESSION['token1']; ?>", function(data, status) {
		...
		function cms_off() {
			...
			$.get("api/cms-off2.php?token1=<?php echo $_SESSION['token1']; ?>", function(data, status) {
				document.getElementById('cms_connecting').style.display = 'none';
				document.getElementById('cms_response').style.display = 'block';
	...
	<style>
		...
		.picture-container .photo-item {
			flex: 1;
			/* 平分空間，效果等同於 w3-half */
			min-width: 300px;
			/* 設定手機版最小寬度，低於此數值會自動換行 */
			padding: 8px;
			/* 增加間距 */
		}
...
<body>
	...
	<div class="main-content w3-main w3-container w3-padding-large">

		<div class="main-card">
			<h1 style="color: #ffffff;">苗62鄉道 6.2K邊坡 即時監測數據</h1>
		</div>

		<div class="main-card w3-card-4" style="margin-top:10px;">
			<header class="main-map w3-padding-large">
			...
			<div class="map-container w3-border-left w3-border-right w3-border-bottom">
				...
				<script type="text/javascript">
					...
					var map = L.map('map', {
						...
						zoom: 17,
						zoomControl: true,
					}).setView([map_marker_list[0].position.lat, map_marker_list[0].position.lng], 17);
					...
					map_marker_list.map(item => L.marker(new L.LatLng(item['position']['lat'], item['position']['lng']), {
							icon: marker_icon,
							zIndexOffset: 5
						}).on('click', function(e) {
							Marker_onClick(item.locpic_num)
						}).addTo(map))
		...
		<div class="main-card w3-card-4" style="margin-top:20px;width:100%;">
			<header class="main-map w3-padding-large">
			...
			<div class="picture-container">
				<div class="w3-half">
					<a href="/manager-page/api/live.php?id=21" target="_blank">
						<img id="img_cam1" loading="lazy" style="width:100%;object-fit:fill;margin-top:14px;" data-src="api/snapshot.php?id=21" src="api/snapshot.php?id=21" onerror="this.src='live/2-1.jpg';" />
					</a>
				</div>
				<div class="w3-half">
					<a href="/manager-page/api/live.php?id=22" target="_blank">
						<img id="img_cam2" loading="lazy" style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;" data-src="api/snapshot.php?id=22" src="api/snapshot.php?id=22" onerror="this.src='live/2-2.jpg';" />
					</a>
				</div>
			</div>
		</div>
		...
		<div class="main-card w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="picture-container">
				<div class="w3-half">
					<img id="img_alert1" style="width:100%;object-fit:fill;margin-top:14px;"
						src="alarm/2-1.jpg<?php echo '?t=' . rand() ?>"
						onerror="this.onerror=null; this.src='alarm/2-1.jpg?t=' + new Date().getTime();"
						loading="lazy">
				</div>
				<div class="w3-half">
					<img id="img_alert2" style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;"
						src="alarm/2-2.jpg<?php echo '?t=' . rand() ?>"
						onerror="this.onerror=null; this.src='alarm/2-2.jpg?t=' + new Date().getTime();"
						loading="lazy">
				</div>
			</div>
		</div>
		...
		<div class="main-card w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="picture-container" style="overflow-x:auto;" id="update_inclinometer">
		...
		<div class="main-card w3-card-4" style="margin-top:20px;">
			<header class="main-map w3-padding-large">
				<svg style="height:16px;" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 512 512">
					<path d="M464 32H48C21.49 32 0 53.49 0 80v352c0 26.51 21.49 48 48 48h416c26.51 0 48-21.49 48-48V80c0-26.51-21.49-48-48-48zM224 416H64v-96h160v96zm0-160H64v-96h160v96zm224 160H288v-96h160v96zm0-160H288v-96h160v96z"></path>
				</svg>
			...
			<div class="picture-container" style="overflow-x:auto;" id="update_raingauge">
				<table class="w3-table-all" style="margin-top:14px;margin-bottom:14px;">
					<tr>
						<th style="text-align:center;vertical-align:middle;">監測位置</th>
						<th style="text-align:center;vertical-align:middle;">燈號狀態</th>
		...
		<div class="main-card w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="picture-container" style="overflow-x:auto;" id="update_power">
				<table class="w3-table-all" style="margin-top:14px;margin-bottom:14px;">
					<tr>
					...
					<?php
					// home_get_line_power(201);
					home_get_battery_voltage(202)
					?>
		...
		<div class="main-card w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="picture-container" style="overflow-x:auto;">
				<table class="w3-table-all" style="margin-top:14px;margin-bottom:14px;">
					<tr>
		...
		<footer class="footer-content w3-panel w3-padding-32 w3-card-4  w3-center w3-opacity">
	...
	<div id="locpic_01" class="w3-modal">
		<div class="w3-modal-content" style="max-width:480px;">
			<div class="w3-bar w3-blue">
				<a class="w3-bar-item w3-padding-8" style="font-size:20px; vertical-align:middle;">苗62鄉道 6.2K邊坡</a>
			...
			<div class="w3-content w3-center">
				<img src="img/locpic_02.jpg" class="w3-light-gray" style="object-fit:contain;width:100%;max-height:360px;">
			</div>
		</div>
	</div>
	...
	<div id="cms_connecting" class="w3-modal">
		...
		<script>
			...
			// 1. 影像更新：一次抓取 4 張圖的最新網址
			function refreshImagesLoop() {
				fetch('fetch_images_2.php?nocache=' + new Date().getTime())
					.then(response => {
					...
					.then(data => {
						// 正常情況：直接套用 API 回傳的網址
						if (data.cam1) document.getElementById('img_cam1').src = data.cam1;
						if (data.cam2) document.getElementById('img_cam2').src = data.cam2;
						if (data.alert1) document.getElementById('img_alert1').src = data.alert1;
						if (data.alert2) document.getElementById('img_alert2').src = data.alert2;
					})
					.catch(err => {
						console.warn("影像 API 發生錯誤，啟用備用機制:", err);
						const fallbackTime = new Date().getTime();

						// 即時擷圖維持本地 API
						document.getElementById('img_cam1').src = 'api/snapshot.php?id=11&t=' + fallbackTime;
						document.getElementById('img_cam2').src = 'api/snapshot.php?id=12&t=' + fallbackTime;

						// 警告擷圖指向外部的 192.168.60.79 伺服器
						document.getElementById('img_alert1').src = 'http://192.168.60.79/manager-page/alarm/2-1.jpg?t=' + fallbackTime;
						document.getElementById('img_alert2').src = 'http://192.168.60.79/manager-page/alarm/2-2.jpg?t=' + fallbackTime;
					})
					.finally(() => {
						setTimeout(refreshImagesLoop, 5000);
					});
			}



			// 2. 燈號更新：改成「循序漸進」模式，避免硬體設備同時收到兩個連線而卡死
			async function updateStatusLightsLoop() {
				try {
					// 第一步：先問「牌面狀態」
					try {
						let cms_res = await $.get("api/cms-status2.php");
						document.getElementById("cms_status_ui").src = cms_res;
						document.getElementById("cms_status_ui2").innerHTML = '🟢';
					} catch (e) {
						document.getElementById("cms_status_ui").src = "img/off1.png";
						document.getElementById("cms_status_ui2").innerHTML = '⚫';
						console.warn("牌面 API 發生錯誤，可能設備忙碌中");
					}

					// 第二步：等牌面狀態問完之後，再問「市電狀態」(完美錯開連線時間)
					try {
						let lp_res = await $.get("api/lp-status2.php");
						var cleanLpData = lp_res.toString().trim();

						if (cleanLpData === "1") document.getElementById("lp_led_status_ui").innerHTML = "🟢";
						else if (cleanLpData === "0") document.getElementById("lp_led_status_ui").innerHTML = "🔴";
						else document.getElementById("lp_led_status_ui").innerHTML = "⚫";
					} catch (e) {
						document.getElementById("lp_led_status_ui").innerHTML = "⚫";
						console.warn("市電 API 發生錯誤，可能設備忙碌中");
					}

				} finally {
					// 不管成功或失敗，都等 10 秒後再排隊執行下一次
					setTimeout(updateStatusLightsLoop, 10000);
				}
			}


			// 獨立出一個專門局部更新表格的函數 (改成排隊迴圈模式)
			function refreshDataTablesLoop() {
				// 產生帶有時間戳記的網址，強制抓取最新資料
				var url = new URL(window.location.href);
				url.searchParams.set('_t', new Date().getTime());

				// 只向伺服器請求「一次」完整的網頁
				$.get(url.toString(), function(htmlData) {
					// 將抓回來的整個網頁暫存到記憶體中，轉換成 jQuery 物件
					var $parsedHTML = $(htmlData);

					// 分別把三個區塊的最新 HTML 挖出來，貼到現在的畫面上
					$("#update_inclinometer").html($parsedHTML.find("#update_inclinometer").html());
					$("#update_raingauge").html($parsedHTML.find("#update_raingauge").html());
					$("#update_power").html($parsedHTML.find("#update_power").html());

					// 注意：這裡不用再呼叫 updateStatusLights() 了，因為燈號已經有自己獨立的 10 秒迴圈在跑，這樣可以再省下伺服器資源！

				}).fail(function() {
					// 加上 fail 保護：萬一伺服器真的沒回應，至少原本的畫面不會被清空消失
					console.warn("資料表更新失敗，伺服器可能正在忙碌中，將於下次重試。");
				}).always(function() {
					// 不管成功或失敗，都等 5 分鐘 (300000 毫秒) 後再排隊執行下一次
					setTimeout(refreshDataTablesLoop, 300000);
				});
			}
			...
			$(function() {
				// 網頁載入後，啟動這三個排隊循環
				refreshImagesLoop(); // 負責即時影像 (每 5 秒)
				updateStatusLightsLoop(); // 負責燈號狀態 (每 10 秒)

				// 負責表格數據 (這裡先等 5 分鐘後才執行第一次，避免網頁剛開就去撞伺服器)
				setTimeout(refreshDataTablesLoop, 300000);
			});
		</script>
</body>
