<?php
    ...
    function home_get_inclinometer_all()
    {
		...
		home_get_inclinometer_with_offset_21("23.5K 傾斜儀(左)");
		home_get_inclinometer_with_offset_22("23.5K 傾斜儀(右)");
		
    	// home_get_inclinometer_with_offset("23.5K 傾斜儀(左)", 21, $id_21_ad1_offset, $id_21_ad2_offset);
    	// home_get_inclinometer_with_offset("23.5K 傾斜儀(右)", 22, $id_22_ad1_offset, $id_22_ad2_offset);
    }
    function home_get_raingauge_all()
    {
        require("C:\\xampp\\htdocs\\default\\lora-offset.php");
    	home_get_raingauge_with_offset("23.5K 雨量筒", 29, $id_29_count_offset);
    }
	
    function home_get_inclinometer_with_offset_21($loc_name)
    {
    	...
    	if ($db_link == TRUE)
    	{
            ...
            $sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=2101 ORDER BY `sn` DESC LIMIT 1;";
            ...
            $sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=2102 ORDER BY `sn` DESC LIMIT 1;";
            $stmt = $db_link->prepare($sql_query);
            if ($stmt == TRUE)
            {
                $stmt->execute();
            ...
            $sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=2105 ORDER BY `sn` DESC LIMIT 1;";
            $stmt = $db_link->prepare($sql_query);
            if ($stmt == TRUE)
            {
                $stmt->execute();
    ...
    function home_get_inclinometer_with_offset_22($loc_name)
    {
		require("C:\\xampp\\htdocs\\default\\variable.php");
        require("C:\\xampp\\htdocs\\default\\ConnMySQL.php");
    	if ($db_link == TRUE)
    	{
            ...
            $sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=2201 ORDER BY `sn` DESC LIMIT 1;";
            $stmt = $db_link->prepare($sql_query);
            if ($stmt == TRUE)
            {
                $stmt->execute();
            ...
            $sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=2202 ORDER BY `sn` DESC LIMIT 1;";
            $stmt = $db_link->prepare($sql_query);
            if ($stmt == TRUE)
            {
                $stmt->execute();
            ...
            $sql_query = "SELECT `time`, `val` FROM `chndata` WHERE `chn_id`=2205 ORDER BY `sn` DESC LIMIT 1;";
            $stmt = $db_link->prepare($sql_query);
            if ($stmt == TRUE)
            {
                $stmt->execute();
    ...
    $output .= '<td style="text-align:center;vertical-align:middle;"><span>' . get_status_circle_raingauge2(2903, strtotime($data_buf["time"])) . '</span></td>' . "\n";
