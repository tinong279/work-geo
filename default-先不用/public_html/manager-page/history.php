				...
				if ($data_count > 0)
				{
				    ...
				    while ($stmt->fetch())
				    {
				        if ($count_buf == 0)
				        {
				            ...
				            if ($data_buf["val"] > $chn_max_val)
				            {
				                $data_buf["val"] = $chn_max_val;
				            }
				            else if ($data_buf["val"] < $chn_min_val)
				            {
				                $data_buf["val"] = $chn_min_val;
				            }
...
<div class="w3-card-4" style="margin-top:20px;min-height:240px;">
    ...
    <div class="w3-container w3-padding-16 w3-border-left w3-border-right w3-border-bottom">
        ...
        <a href="download.php?<?php echo 'chn=' . $chn . '&date_start=' . $date_start_timestamp . '&date_stop=' . $date_stop_timestamp; ?>" target="_blank" class="w3-button w3-padding w3-round w3-blue" style="width:80px;font-size:16px;margin-top:8px;">下載</a>
        <a href="datatable.php?<?php echo 'chn=' . $chn . '&date_start=' . $date_start_timestamp . '&date_stop=' . $date_stop_timestamp; ?>" target="_blank" class="w3-button w3-padding w3-round w3-blue" style="width:100px;font-size:16px;margin-top:8px;">原始數據</a>
    </div>
</div>
<script>
const x = [<?php echo $all_time; ?>];
const y = [<?php echo $all_val; ?>];
...
<?php
    ...
    if ($submit_flag == true)
    {
        $str_buf1 = $json_obj->{"chart_yaxis_min"}->{$chn};
        $str_buf2 = $json_obj->{"chart_yaxis_max"}->{$chn};
        ...
        else if ($chn_type_flag == 5 || $chn_type_flag == 8)
        {
            $str_buf3 = '電壓';
        }
...
?>
                    min: <?php echo $str_buf1; ?>,
                    max: <?php echo $str_buf2; ?>,
                    maxTicksLimit: 12,
                    fontSize: 16,
                },
                scaleLabel:
