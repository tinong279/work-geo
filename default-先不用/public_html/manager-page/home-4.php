<?php
    ...
    require("C:\\xampp\\htdocs\\default\\home-func-4.php");
    require("C:\\xampp\\htdocs\\default\\public-func.php");
?>
<!DOCTYPE html>
<html lang="zh-tw">
	<head>
		<title>苗縣道126 28.6K邊坡 即時監測數據 - <?php echo $system_name; ?></title>
		<meta charset="utf-8">
...
<?php get_css_js_link() ?>
		...
		function cms_on()
		{
			...
			$.get("api/cms-on4.php?token1=<?php echo $_SESSION['token1']; ?>", function(data, status)
			{
		...
		function cms_off()
		{
			...
			$.get("api/cms-off4.php?token1=<?php echo $_SESSION['token1']; ?>", function(data, status)
			{
				document.getElementById('cms_connecting').style.display='none';
				document.getElementById('cms_response').style.display='block';
...
<script type="text/javascript">
    ...
    function Marker_onClick(obj_id)
    {
		...
		<div class="w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="w3-container w3-border-left w3-border-right w3-border-bottom" >
                <div class="w3-half" style="">
					<!--
                    <img style="width:100%;object-fit:fill;margin-top:14px;" src="live/4-1.jpg<?php echo '?t=' . rand() ?>" loading="lazy"></img>
					-->
					<a href="/manager-page/api/live.php?id=41" target="_blank">
					<img loading="lazy" style="width:100%;object-fit:fill;margin-top:14px;" data-src="/manager-page/api/snapshot.php?id=41" src="/manager-page/api/snapshot.php?id=41" onerror="this.src='live/4-1.jpg';" class="video_obj" />
					</a>
				</div>
                <div class="w3-half" style="">
					<!--
                    <img style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;" src="live/4-2.jpg<?php echo '?t=' . rand() ?>" loading="lazy"></img>
					-->
					<a href="/manager-page/api/live.php?id=42" target="_blank">
					<img loading="lazy" style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;" data-src="/manager-page/api/snapshot.php?id=42" src="/manager-page/api/snapshot.php?id=42" onerror="this.src='live/4-2.jpg';" class="video_obj" />
					</a>
				</div>
			</div>
		</div>
		...
		<div class="w3-card-4" style="margin-top:20px;width:100%;">
			...
			<div class="w3-container w3-border-left w3-border-right w3-border-bottom" >
                <div class="w3-half" style="">
                    <img style="width:100%;object-fit:fill;margin-top:14px;" src="alarm/4-1.jpg<?php echo '?t=' . rand() ?>" loading="lazy"></img>
                </div>
                <div class="w3-half" style="">
                    <img style="width:100%;object-fit:fill;margin-top:14px;margin-bottom:14px;" src="alarm/4-2.jpg<?php echo '?t=' . rand() ?>" loading="lazy"></img>
                </div>
			</div>
		</div>
...
<?php
	// home_get_line_power(401);
	home_get_battery_voltage(402)
?>
...
<span id="cms_status_ui2">⚫</span>
	...
	<div id="locpic_01" class="w3-modal">
		<div class="w3-modal-content" style="max-width:480px;">
			<div class="w3-bar w3-blue">
				<a class="w3-bar-item w3-padding-8" style="font-size:20px; vertical-align:middle;">苗縣道126 28.6K邊坡</a>
			...
			<div class="w3-content w3-center">
				<img src="img/locpic_04.jpg" class="w3-light-gray" style="object-fit:contain;width:100%;max-height:360px;">
			</div>
		</div>
	</div>
	...
	<script>
		...
		$.get("api/cms-status4.php", function(data, status)
		{
			if (status == "success")
			{
		...
		$.get("api/lp-status4.php", function(data, status)
		{
			if (status == "success")
			{
				if (data == "1")
