...
require_once '../api/ConnMySQL.php';

// 查詢可用的圖片
$image_list = [];
if (!isset($db_link) || $db_link->connect_error) {
    // 如果資料庫沒連上，使用預設圖片
    $image_list = [
    ...
    $sql = "SELECT filename, file_path, category 
            FROM cms_images 
    ...
    try {
        ...
        if ($action == 'publish') {
            ...
            if ($display_type == 'text') {
                ...
                $stmt->bind_param("ssii", $text_content, $text_color, $text_size, $cms_id);
                $stmt->execute();
                ...
                if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] == 0) {
                    ...
                    if (move_uploaded_file($_FILES['image_file']['tmp_name'], $target_path)) {
                        ...
                        $sql = "INSERT INTO cms_images (filename, file_path, category, uploaded_by) 
                                VALUES (?, ?, 'manual', ?)";

                        $stmt = $db_link->prepare($sql);

                        if ($stmt === false) {
                            die("SQL prepare 失敗：" . $db_link->error . "<br>SQL: " . $sql);
                        }

                        $stmt->bind_param("sss", $filename, $image_path, $uploaded_by);
                        $stmt->execute();
                    }
                } else {
                ...
                if ($image_path) {
                    ...
                    $stmt = $db_link->prepare($sql);
                    $stmt->bind_param("si", $image_path, $cms_id);
                    $stmt->execute();
...
try {
    ...
    $sql = "SELECT id, name, ip_address, current_mode, display_type,
    ...
    if ($result === false) {
        throw new Exception($db_link->error);
    }
